...
C4512::C4512(const std::string& name)
    : AComponent(name, 16)
{
    m_pins[0] = Pin(Pin::Type::INPUT);   // X0 (Data input 0)
    m_pins[1] = Pin(Pin::Type::INPUT);   // X1 (Data input 1)
    m_pins[2] = Pin(Pin::Type::INPUT);   // X2 (Data input 2)
    m_pins[3] = Pin(Pin::Type::INPUT);   // X3 (Data input 3)
    m_pins[4] = Pin(Pin::Type::INPUT);   // X4 (Data input 4)
    m_pins[5] = Pin(Pin::Type::INPUT);   // X5 (Data input 5)
    m_pins[6] = Pin(Pin::Type::INPUT);   // X6 (Data input 6)
    m_pins[8] = Pin(Pin::Type::INPUT);   // X7 (Data input 7)
    m_pins[9] = Pin(Pin::Type::INPUT);   // Inhibit
    m_pins[10] = Pin(Pin::Type::INPUT);  // A (Address bit 0)
    m_pins[11] = Pin(Pin::Type::INPUT);  // B (Address bit 1)
    m_pins[12] = Pin(Pin::Type::INPUT);  // C (Address bit 2)
    m_pins[14] = Pin(Pin::Type::INPUT);  // Strobe
